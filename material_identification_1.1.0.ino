...
delay(1000); ////////////////////////////////////////////////delay between detecting and scanning a material /////////////////////////////////////////////////////
...
else
{
  ...
  if (digitalRead(trainingpin)){
    ...
    //normalise(spectrum); //only used if readings not proportional to calibrated averages //
...
void get_spectral_readings(float a[]){
  int i;
  a[0]=spectrumaverage[0]/getspectra(LED850);
  a[1]=spectrumaverage[1]/getspectra(LED1050);
  a[2]=spectrumaverage[2]/getspectra(LED1200);
  a[3]=spectrumaverage[3]/getspectra(LED1310);
  a[4]=spectrumaverage[4]/getspectra(LED1450);
  a[5]=spectrumaverage[5]/getspectra(LED1550);
  a[6]=spectrumaverage[6]/getspectra(LED1600);
  for(i=0;i<7;i++){
    abc[i]=a[i];
  }
}
...
char getmostlikely(float a[]){
  ...
  while (myfile.available()){
    count3++; //line counter. For debugging and stuff.
    datain_char=(myfile.readStringUntil(','))[0]; //The function returns an array of characters. Use [0] to remove the comma that may come with it.
    datain[0]=(myfile.readStringUntil(',')).toFloat();
    datain[1]=(myfile.readStringUntil(',')).toFloat(); 
    datain[2]=(myfile.readStringUntil(',')).toFloat();
    datain[3]=(myfile.readStringUntil(',')).toFloat(); //toFloat removes the comma automatically, as it's not part of a recognised float format.
    datain[4]=(myfile.readStringUntil(',')).toFloat();
    datain[5]=(myfile.readStringUntil(',')).toFloat();
    datain[6]=(myfile.readStringUntil(',')).toFloat();
    //normalise(datain); //only if not using calibrated proportions.
    myfile.readStringUntil('\n');
    proportioned[0]=abs(datain[0]-a[0]);
    proportioned[1]=abs(datain[1]-a[1]);
    proportioned[2]=abs(datain[2]-a[2]);
    proportioned[3]=abs(datain[3]-a[3]);
    proportioned[4]=abs(datain[4]-a[4]);
    proportioned[5]=abs(datain[5]-a[5]);
    proportioned[6]=abs(datain[6]-a[6]);

    distance=0; //reset distance, as it will be increased in the following 'for' loop.
    for(j=0;j<7;j++){
      proportioned[j]=pow((proportioned[j]),2);
  ...
  myfile.close();
  Serial.print(temp); //show what the probability of error was
  Serial.println(F(" << Higher number = less sure about outcome."));
  //if (temp>0.3){
  //  result = 'n';
  //}
  //if (temp>0.2){
